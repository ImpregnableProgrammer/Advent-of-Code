# Part One
First_Part=lambda g:[all(o not in f for o in['ab','cd','pq','xy'])and any(i in f for i in[p*2for p in'abcdefghijklmnopqrstuvwxyz'])and sum([f.count(i)for i in'aeiou'])>=3for f in g.split('\n')].count(1)
